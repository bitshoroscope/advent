...
def get_life_support_rating():
    with open('../../resources/debug_input.txt') as f:
        ...
        for word in transposed_lines:
            counter_ones, counter_zeros = get_bit_counters(word)
            filters_oxygen.append('1' if counter_ones >= counter_zeros else '0')
            filters_co2.append('0' if counter_zeros <= counter_ones else '1')
...
def get_rating(filtered_lines, filters):
    index = 0
    for filter in filters:
        filtered_lines = [line for line in filtered_lines if line[index] == filter]
